describe('Fluxo de exclusão de um agendamento', () => {
    const usuario = {
    ...
    it('deve criar um agendamento inicial para ser excluido', () => {
        cy.window().then((win) => {
    ...
    it('deve excluir o agendamento', () => {
        ...
        cy.get('button.bg-red-600', { timeout: 10000 }).first().click({ force: true });

        cy.contains('Deseja realmente excluir?', { timeout: 10000 }).should('be.visible');

        cy.contains('Confirmar').click();

        cy.contains('Agendamento excluído com sucesso', { timeout: 10000 }).should('exist');
    });
